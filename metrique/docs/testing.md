...
For simple tests where you just want to verify field values without setting up a sink, [`test_metric`](https://docs.rs/metrique/latest/metrique/test_util/fn.test_metric.html) closes a metric struct and returns a [`TestEntry`](https://docs.rs/metrique/latest/metrique/test_util/struct.TestEntry.html) you can assert against directly:
...
`metrique` provides `test_entry_sink` which allows introspecting the entries that are emitted (without needing to read EMF directly). You can use this functionality in combination with the [`TestEntrySink`](https://docs.rs/metrique/latest/metrique/test_util/struct.TestEntrySink.html) to test that you are emitting the metrics that you expect:
...
[`LocalFormat`](https://docs.rs/metrique/latest/metrique/local/struct.LocalFormat.html) renders metric entries in a readable
