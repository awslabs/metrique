...
- `sysinfo-bridge`: Subscribes [sysinfo] system and current-process snapshots to a global entry sink — CPU usage, memory and swap, load average, uptime, and current-process memory/CPU/disk/runtime/open-file counters. Disk space, network rx/tx, and thermal sensor metrics are opt-in via `SysinfoMetricsConfig::with_disks` / `with_networks` / `with_components`. The reporter task is automatically aborted when the `AttachHandle` is dropped.
